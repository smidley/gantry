...
- **Nvidia path — optional:** template variable flips on `--runtime=nvidia` + `NVIDIA_VISIBLE_DEVICES=all`; Gantry execs the runtime-injected `nvidia-smi` (CSV queries) if present — dlopen would require cgo or an FFI dependency, and exec keeps the static build; host utilization + per-process memory → same PID→container mapping. Per-container Nvidia in v1 is VRAM + presence only (CSV has no per-process SM-utilization column); host-level `utilization.gpu` is still collected. Runtime absent ⇒ Nvidia panel shows the enable hint.
