...
- **2026-08-25 (Task 18 on-box validation):** root-caused — that "host-side" client was actually containerized; Gantry's default private cgroupns strips the `docker/` path component before `ContainerID` ever sees it, so the id relativizes to a bare hex segment the old regex didn't match (fixed in `internal/collect/cgroup`). The host bucket itself remains correct and stays for genuinely host-side clients.
