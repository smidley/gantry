...
// diskNotPresent is disks.ini's status value for an empty array slot.
// Every other status (DISK_OK today; disabled/invalid states in later
// Unraid versions) counts as present — per-disk metrics gate on "not
// DISK_NP", not on an allowlist of known-good statuses.
...
func (c *Collector) tickOneDisk(slot string, disk map[string]string, ts int64) {
	...
	if sizeOK && freeOK && fsSize > 0 {
		c.sink.Record(store.SeriesKey{Kind: "disk", Entity: slot, Metric: "fs.used_bytes"}, ts, (fsSize-fsFree)*1024)
