...
export interface GraceState<T> {
  lastSeenRow: Map<string, T>;
  lastPresentTick: Map<string, number>;
  tick: number;
}

// GRACE_TICKS: one extra tick (2s of real cadence, main.go's own 2s
// publish ticker) is comfortably longer than TopBarList's 250ms
// transition needs to fully settle, so a same-tick flicker never
// reaches Svelte's transition system as a removal at all -- see
// withGracePeriod's own doc for why that matters.
const GRACE_TICKS = 1;

// withGracePeriod keeps a row that just dropped out of `rows` displayed,
// at its last known value, for GRACE_TICKS more calls -- topFromFrame's
// own top-N cut has no memory of its own (a deliberate, stateless
// snapshot, same contract as the server's topFromSnapshot it mirrors),
// so a container hovering right at the cutoff can cross it on nearly
// every tick. Handing THAT straight to the {#each} block as a real
// removal is what breaks TopBarList's own animate:flip/transition:fade
// (Scott: "list changing places... not doing so with a smooth
// animation"): a key that leaves then immediately comes back makes
// Svelte link the new intro to the still-running outro as its
// "counterpart" so a reversed transition can resume smoothly instead of
// jumping -- but the resumed duration is computed proportional to how
// far the OLD transition actually got, barely anywhere for a flicker
// this fast, so it rounds to an instant, invisible pop; the row also
// never cleanly finishes that outro (confirmed live: a dead, opacity:0
// `<li>` the page then carries forever, one per flicker). A key gone
// for longer than GRACE_TICKS is forgotten outright (not left to grow
// the map forever) -- a later reappearance just starts fresh, same as
// an entity that's never been seen before. Same "a flicker at the edge
// shouldn't read as two events" shape as reorderByLastDisplayedValue
// above, for MEMBERSHIP instead of rank; `state` is the caller's own
// persistent, per-metric bookkeeping (see TopBarList.svelte's own call
// site), mirroring reorderByLastDisplayedValue's `previous` Map.
export function withGracePeriod<T extends { entity: string }>(
  rows: T[],
  state: GraceState<T>,
  metricKey: string,
  ...
  const keyOf = (entity: string) => prefix + entity;

  for (const row of rows) {
    const key = keyOf(row.entity);
    state.lastSeenRow.set(key, row);
    state.lastPresentTick.set(key, state.tick);
  }

  const currentKeys = new Set(rows.map((r) => keyOf(r.entity)));
  const graced: T[] = [];
  for (const [key, lastTick] of [...state.lastPresentTick]) {
    if (!key.startsWith(prefix)) continue; // another metric's own bookkeeping, sharing this Map
    if (currentKeys.has(key)) continue;
    if (state.tick - lastTick <= graceTicks) {
      graced.push(state.lastSeenRow.get(key) as T);
    } else {
      state.lastPresentTick.delete(key);
      state.lastSeenRow.delete(key);
    }
  }

  return graced.length === 0 ? rows : [...rows, ...graced];
}
