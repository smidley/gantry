<!--
  ...
  Evidence is a PAGE of its own now (#/insights/:id, views/
  InsightDetail.svelte) rather than the modal drawer this view used to
  own -- the owner's own ask, "the evidence pop up on the insights page
  should just open a whole new page instead of trying to pack all of the
  info into the little popup." Every opener here is therefore a real
  link (an active row, a history row) or a plain hash navigation (a map
  edge), so middle-click/new-tab/back all behave the way they do
  everywhere else in this app, and this view fetches nothing per click
  at all.
...
<script>
  ...
  import { fetchInsightHistory, fetchInsightRules, putInsightRules, dismissInsight, fetchInsightGraph } from '../lib/api';
  import { sortActiveInsights, confidenceLabel, activeDuration, describeRule } from '../lib/insights';
  ...
  // openInsight is the MAP's own edge handler: the list/history rows
  // are plain <a href> (see the template), which is what a navigation
  // should be, but a map edge is an SVG <g> with its own keyboard
  // contract already (InteractionMap's own doc) -- it navigates the
  // hash directly rather than being wrapped in an anchor.
  function openInsight(id) {
    window.location.hash = `#/insights/${id}`;
  }
...
<div class="insights-view">
  ...
  {#if mode === 'map'}
    <div class="card insights-view__map">
      <InteractionMap {graph} statementsById={statementsByID} {tier} onOpenInsight={openInsight} />
    </div>
  {:else}
          ...
          {#each active as i (i.id)}
            ...
            <li class="insights-view__row">
              ...
              <div class="insights-view__row-body">
                <a class="insights-view__statement-link" href="#/insights/{i.id}">{i.statement}</a>
      ...
      <ul class="insights-view__rows">
        {#each history as h (h.id)}
          <li class="insights-view__row insights-view__row--history">
            ...
            <div class="insights-view__row-body">
              <a class="insights-view__statement-link" href="#/insights/{h.id}">{h.statement}</a>
              <div class="insights-view__row-meta">
...
<style>
  ...
  /* A real link to the finding's own evidence page (#/insights/:id),
     styled as the plain statement text it replaced -- middle-click and
     "open in new tab" have to work, which is exactly what the button
     this used to be could not do. */
  .insights-view__statement-link {
    align-self: flex-start;
    font-weight: 500;
    color: var(--ink);
  }
  .insights-view__statement-link:hover {
